...
FROM (
    SELECT p.first_name, p.last_name, c.height
    FROM cards AS c,
         patients AS p
...
FROM (
    ...
    FROM patients AS p
    JOIN cards AS c ON p.patient_id = c.patient_id
     ) AS bmi_stats
...
SELECT concat(p.first_name, ' ', p.last_name) AS patient_fullname,
       ...
       d.qualification, d.specialization, a.possible_diagnosis
FROM doctors AS d
...
SELECT concat(p.first_name, ' ', p.last_name) AS patient_fullname,
       c.established_diagnosis, c.treatment_method, c.sum_to_pay
FROM patients AS p
